#!/bin/bash
# Entrypoint for the ROS1 image publisher container.
#
# Responsibilities:
#   1. Source the ROS env so rospy / sensor_msgs are importable.
#   2. exec main.py — config comes from YAML, not flags.
#
# Note: this container does NOT start a roscore. The publisher and the
# bridge are deliberately split; bring up `roscore` separately (in a
